...
pub struct RunState {
    pub(crate) current_cell_x: u32,
    current_cell_y: u32,
    pub(crate) current_x_in_cell: f64,
    pub(crate) current_y_in_cell: f64,
...
impl RunState {
    ...
    pub fn global_y(&self) -> f64 {
        self.current_cell_y as f64 + self.current_y_in_cell
    }
    ...
    fn will_collide_with_wall(&self, new_x: f64, new_y: f64, maze: &MazePhenotype) -> bool {
        ...
        if cell.north_wall && new_x % 1.0 < config::AGENT.agent_radius {
            return true;
        }
        if cell.south_wall && new_x % 1.0 < config::AGENT.agent_radius {
            return true;
        }
